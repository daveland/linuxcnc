...
int genhex_hal_setup(int comp_id)
{
    int i,res;

    haldata = hal_malloc(sizeof(struct haldata));
    if (!haldata) {
        rtapi_print_msg(RTAPI_MSG_ERR,"genhex_hal_setup: hal_malloc fail\n");
        return -1;
    }
    ...
    *haldata->tool_offset = 0.0;
    ...
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_x, comp_id,
        "genhexkins.gui.x")) < 0)
    goto error;
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_y, comp_id,
        "genhexkins.gui.y")) < 0)
    goto error;
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_z, comp_id,
        "genhexkins.gui.z")) < 0)
    goto error;
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_a, comp_id,
        "genhexkins.gui.a")) < 0)
    goto error;
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_b, comp_id,
        "genhexkins.gui.b")) < 0)
    goto error;
    if ((res = hal_pin_float_newf(HAL_IN, &haldata->gui_c, comp_id,
        "genhexkins.gui.c")) < 0)
    goto error;

    if ((res = hal_pin_bit_newf(HAL_OUT, &haldata->fwd_kins_fail, comp_id,
        "genhexkins.fwd-kins-fail")) < 0)
    goto error;
...
int genhex_gui_forward_kins(const double *joints, EmcPose *pos)
{
    int res;
    KINEMATICS_FORWARD_FLAGS  fflags;
    KINEMATICS_INVERSE_FLAGS  iflags;
    res = genhexKinematicsForward(joints,pos,&fflags,&iflags);
